...
class Settings:
    ...
    capital_scaling: bool = True           # раз в неделю капитал перетекает к прибыльным трейдерам
    capital_min_mult: float = 0.4          # счёт не меньше 0.4 × стартового
    capital_max_mult: float = 3.0          # и не больше 3 × стартового
    push_subject: str = "mailto:botz@example.com"   # контакт для push-службы (VAPID)
    briefing_hour_utc: int = 1             # утренний брифинг в 06:00 по Астане
    live_enabled: bool = False             # зеркалить сделки трейдеров со званием «Реальный счёт» на бирже
    live_testnet: bool = True              # тестовая сеть Binance (виртуальные деньги, настоящие ордера)
    live_api_key: str = ""
    live_api_secret: str = ""
    live_capital_usd: float = 100.0        # сколько реальных долларов повторяют каждый демосчёт трейдера с реальным званием
...
def load_settings(env_file: str | Path | None = ".env") -> Settings:
    ...
    return Settings(
        ...
        capital_scaling=_bool(env.get("CAPITAL_SCALING"), True),
        capital_min_mult=float(env.get("CAPITAL_MIN_MULT", "0.4")),
        capital_max_mult=float(env.get("CAPITAL_MAX_MULT", "3.0")),
        push_subject=env.get("PUSH_SUBJECT", "mailto:botz@example.com"),
        briefing_hour_utc=int(env.get("BRIEFING_HOUR_UTC", "1")),
        live_enabled=_bool(env.get("LIVE_ENABLED"), False),
        live_testnet=_bool(env.get("LIVE_TESTNET"), True),
        live_api_key=env.get("LIVE_API_KEY", ""),
        live_api_secret=env.get("LIVE_API_SECRET", ""),
        live_capital_usd=float(env.get("LIVE_CAPITAL_USD", "100")),
